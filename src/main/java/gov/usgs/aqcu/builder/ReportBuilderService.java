...
import gov.usgs.aqcu.model.SensorReadingSummaryReportMetadata;
...
public class ReportBuilderService {
	...
	@Autowired
	public ReportBuilderService(FieldVisitDataService fieldVisitDataService, 
	...
	public SensorReadingSummaryReport buildReport(SensorReadingSummaryRequestParameters requestParameters, String requestingUser) {
		SensorReadingSummaryReport srsReport = new SensorReadingSummaryReport();
	...
	protected TimeSeriesCorrectedData buildTimeSeriesCorrectedData(
			...
			SensorReadingSummaryRequestParameters requestParameters, Map<String, ParameterMetadata> parameterMetadata) {
	...
	protected SensorReadingSummaryReportMetadata createDvHydroMetadata(SensorReadingSummaryRequestParameters requestParameters,
	...
	protected List<FieldVisitMeasurement> buildFieldVisitMeasurements(SensorReadingSummaryRequestParameters requestParameters,
	 ...
	 */
	protected List<SensorReadingSummaryPoint> createDvHydroPoints(List<TimeSeriesPoint> timeSeriesPoints,
			boolean isDaily, ZoneOffset zoneOffset) {
		List<SensorReadingSummaryPoint> dvPoints = timeSeriesPoints.parallelStream()
				.filter(x -> x.getValue().getNumeric() != null)
				.map(x -> {
					SensorReadingSummaryPoint dvPoint = new SensorReadingSummaryPoint();

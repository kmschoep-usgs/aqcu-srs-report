...
public class NearestTimePointCalculatorTest {
	...
	@MockBean
	private static final SensorReadingSummaryReading reading1 = new SensorReadingSummaryReading();
	private static final SensorReadingSummaryReading reading2 = new SensorReadingSummaryReading();
	private static final SensorReadingSummaryReading reading3 = new SensorReadingSummaryReading();
	...
	private static final List<SensorReadingSummaryReading> readings_date_order = Arrays.asList(reading1, reading2, reading3);
	private static final List<SensorReadingSummaryReading> readings_out_of_date_order = Arrays.asList(reading2, reading1, reading3);
	
	@Test
	public void test_nearest_corrected_point_with_quals_readings_in_date_order() {
		
		List<SensorReadingSummaryReading> nearestPointResults = new NearestTimePointCalculator().findNearest(readings_date_order, TS_DATA_RESPONSE_QUALS, true);
		
		assertEquals(Instant.parse("2013-10-28T09:51:00Z"), nearestPointResults.get(0).getNearestCorrectedTime());
		assertEquals("6", nearestPointResults.get(0).getNearestCorrectedValue());
		assertEquals(1, nearestPointResults.get(0).getQualifiers().size());
		assertEquals(qualifier1, nearestPointResults.get(0).getQualifiers().get(0));
		assertEquals(Instant.parse("2013-10-28T10:20:00Z"), nearestPointResults.get(1).getNearestCorrectedTime());
		assertEquals("8", nearestPointResults.get(1).getNearestCorrectedValue());
		assertEquals(1, nearestPointResults.get(1).getQualifiers().size());
		assertEquals(qualifier1, nearestPointResults.get(1).getQualifiers().get(0));
		assertEquals(Instant.parse("2013-10-28T10:35:00Z"), nearestPointResults.get(2).getNearestCorrectedTime());
		assertEquals("9", nearestPointResults.get(2).getNearestCorrectedValue());
		assertEquals(1, nearestPointResults.get(2).getQualifiers().size());
		assertEquals(qualifier1, nearestPointResults.get(2).getQualifiers().get(0));
	...
	public void test_nearest_corrected_point_without_quals_readings_in_date_order() {
		...
		List<SensorReadingSummaryReading> nearestPointResults = new NearestTimePointCalculator().findNearest(readings_date_order, TS_DATA_RESPONSE_NO_QUALS, true);
	...
	public void test_nearest_corrected_point_with_quals_readings_not_in_date_order() {
		...
		List<SensorReadingSummaryReading> nearestPointResults = new NearestTimePointCalculator().findNearest(readings_out_of_date_order, TS_DATA_RESPONSE_QUALS, true);
		
		assertEquals(Instant.parse("2013-10-28T10:20:00Z"), nearestPointResults.get(0).getNearestCorrectedTime());
		assertEquals("8", nearestPointResults.get(0).getNearestCorrectedValue());
		assertEquals(Instant.parse("2013-10-28T09:51:00Z"), nearestPointResults.get(1).getNearestCorrectedTime());
		assertEquals("6", nearestPointResults.get(1).getNearestCorrectedValue());
		assertEquals(Instant.parse("2013-10-28T10:35:00Z"), nearestPointResults.get(2).getNearestCorrectedTime());
		assertEquals("9", nearestPointResults.get(2).getNearestCorrectedValue());
